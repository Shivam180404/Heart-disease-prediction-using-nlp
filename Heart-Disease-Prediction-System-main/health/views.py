...
def add_heartdetail(request):
    ...
    if request.method == "POST":
        try:
            sex_value = request.POST.get("sex", "").strip().lower()
            if sex_value in ["male", "m", "1"]:
                ...
                error = "sex_invalid"
                return render(request, 'add_heartdetail.html', {'error': error})
        ...
        rem = int(pred[0])
        print("Result = ",rem)
        # Dataset label meaning in this project: 0 = heart disease risk, 1 = healthy.
        if pred[0] == 0:
...
def predict_desease(request, pred, accuracy):
    patient = Patient.objects.get(user=request.user)
    patient_location = (patient.address or "").strip()
    patient_lat = patient.latitude
    patient_lon = patient.longitude

    def distance_in_km(lat1, lon1, lat2, lon2):
        # Haversine distance for geo-based nearest recommendations.
    ...
    all_doctors = Doctor.objects.filter(status=1).select_related('user')
    doctor_with_distance = []
    for doc in all_doctors:
        dist = distance_in_km(patient_lat, patient_lon, doc.latitude, doc.longitude)
        doctor_with_distance.append({"obj": doc, "distance_km": dist})

    if patient_lat is not None and patient_lon is not None:
        doctor_with_distance.sort(key=lambda x: (x["distance_km"] is None, x["distance_km"]))
        nearby_doctors = [item for item in doctor_with_distance if item["distance_km"] is not None][:10]
    else:
        fallback = all_doctors.filter(address__icontains=patient_location)
        nearby_doctors = [{"obj": doc, "distance_km": None} for doc in fallback]

    cardiologists = [item for item in nearby_doctors if "cardio" in (item["obj"].category or "").lower()]
    if not cardiologists:
        cardiologists = nearby_doctors

    all_hospitals = Hospital.objects.all()
    hospital_with_distance = []
    for hospital in all_hospitals:
        dist = distance_in_km(patient_lat, patient_lon, hospital.latitude, hospital.longitude)
        hospital_with_distance.append({"obj": hospital, "distance_km": dist})

    if patient_lat is not None and patient_lon is not None:
        hospital_with_distance.sort(key=lambda x: (x["distance_km"] is None, x["distance_km"]))
        nearby_hospitals = [item for item in hospital_with_distance if item["distance_km"] is not None][:10]
    else:
        nearby_hospitals = [
            {"obj": hospital, "distance_km": None}
            for hospital in all_hospitals.filter(address__icontains=patient_location)
        ]

    d = {
        'pred': pred,
        'accuracy': accuracy,
        'doctor': nearby_doctors,
        'cardiologists': cardiologists,
        'nearby_hospitals': nearby_hospitals,
        'patient_location': patient_location,
        'patient_has_coords': patient_lat is not None and patient_lon is not None,
    }
